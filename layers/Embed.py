...
class EXPatchEmbedding(nn.Module):
    def __init__(self, d_model, patch_len, dropout):
        super(EXPatchEmbedding, self).__init__()
        # Patching
        self.patch_len = patch_len
...
class GRUmodel(nn.Module):
    def __init__(self, inputsize, hiddensize):
        ...
        self.rnn = nn.GRU(inputsize, hiddensize, 1, batch_first=True)
        self.relu = nn.ReLU()
...
class LSTMmodel(nn.Module):
    def __init__(self, inputsize, hiddensize):
        ...
        self.rnn = nn.LSTM(inputsize, hiddensize, 1, batch_first=True)
        self.relu = nn.ReLU()
...
class EXPatchCNN(nn.Module):
    def __init__(self, t_model,d_model, patch_len, dropout):
        super(EXPatchCNN, self).__init__()
        # Patching
        self.patch_len = patch_len
        self.d_model = d_model
        self.t_model = t_model
        # Backbone, Input encoding: projection of feature vectors onto a d-dim vector space
        self.convs = nn.Conv2d(in_channels=1, out_channels=self.t_model, kernel_size=(1, 3), padding=(0, 1))
        self.normalize_layers = nn.BatchNorm2d(self.t_model)
        self.dropout = nn.Dropout(dropout)
        self.temporal = GRUmodel(self.t_model, self.d_model)

    def forward(self, x):
        # do patching
        b, n, t = x.shape
        x = x.unfold(dimension=-1, size=self.patch_len, step=self.patch_len)
        x = x.transpose(1, 2).reshape(-1, n, self.patch_len)

        x = self.convs(torch.unsqueeze(x, 1))
        x = self.normalize_layers(x)
        x = x.permute(0, 2, 3, 1).reshape(b * t // self.patch_len * n, self.patch_len, self.t_model)
        x = self.temporal(x).squeeze().reshape(b, t // self.patch_len, n, self.d_model)
